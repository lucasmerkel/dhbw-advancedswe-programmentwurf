...
Bei den Unit-Tests wurde ein besonderer Fokus auf die Validierungs-Klassen in der \textit{Abstraktions}-Schicht sowie das Erzeugen und Ändern einzelner Attribute eines \href{}{\code{ConsumerGoods}} gelegt.
...
Die Testklasse \href{}{\code{DayDateValidationTest}} überprüft die Funktionalität der Klasse \href{}{\code{DayValidator}}, indem eine Validierung eines gültigen als auch eines negativen oder eines über 31 liegenden Wertes gestestet wird.
Die Testklasse \href{}{\code{MonthDateValidationTest}} überprüft die Funktion der Klasse \href{}{\code{MonthValidator}}, indem eine Validierung eines gültigen als auch eines negativen oder eines über 12 liegenden Wertes gestestet wird.
Die Testklasse \href{}{\code{YearDateValidationTest}} überprüft die Funktion der Klasse \href{}{\code{YearValidator}}, indem eine Validierung eines gültigen als auch eines negativen Wertes gestestet wird.
Die Testklasse \href{}{\code{DateValidationTest}} überprüft die Funktion der Klasse \code{DateValidator}, indem eine Validierung eines gültigen als auch eines ungültigen Datums im Februar und an einem Monat mit 30 statt 31 Tagen gestestet wird.
...
Die Testklasse \href{}{\code{UnitOfMeasureValueValidationTest}} überprüft die Funktion der Klasse \href{}{\code{ValueValidator}}, indem eine Validierung eines gültigen als auch eines negativen Wertes gestestet wird.

Die Testklasse \href{}{\code{StoreConsumerGoodsTest}} überprüft die Klasse \href{}{\code{ConsumerGoodsBuilder}} sowie dessen Validierungsmethode, indem unterschiedliche Kombinationen mit invaliden Übergabeparametern getestet werden.

Die Testklasse \href{}{\code{ConsumerGoodsGuiControllerTest}} dient der Überprüfung der \href{}{\code{\ac{HTTP}-Statuscodes}} für erfolgreiche als auch fehlerhafte Anfragen zum Erhalt, der Erzeugung dem Aktualisieren oder Löschen von ConsumerGoods.

Der Schwerpunkt der genannten Klassen, mit Ausnahme von \href{}{\code{ConsumerGoodsGuiControllerTest}}, liegt auf der Wertevalidierung als Grundlage zur Vermeidung von auftretenden Fehlern, die zu einem Fehlverhalten der Software führen können, das unter Umständen nicht direkt bemerkt wird oder auch durch den Tausch verschiedener Peripherie auftreten könnte.
...
Als Beispiel für die Erfüllung der \textit{Automic}-Regel dient das Nutzen von Assertions wie in \href{}{\code{DateValidatorTest}} zu erkennen.
...
Die \textit{Thorough}-Regel wurde insofern eingehalten, dass der Schwerpunkt der Tests auf den Validierungsklassen, wie dem \href{}{\code{DateValidatorTest}} oder dem \href{}{\code{UnitOfMeasureValueTest}} gelegt wurde.
Gleiches gilt für \href{}{\code{StoreConsumerGoodsTest}}, indem der Schwerpunkt auf die Validierung der Attribute zum Erzeugen eines Konsumguts liegt.
...
Ein weiteres Beispiel für das Einhalten der \textit{Repeatable}-Regel ist ebenfalls im \textit{Arrange}-Teil des Tests \href{}{\code{DateValidatorTest}} dargestellt.
...
Alle nötigen Informationen zum unabhängigen Durchführen des Tests sind im \textit{Assert}-Teil des Tests definiert, wie in \href{}{\code{DateValidatorTest}} beispielhaft dargestellt.
Zur Erfüllung der \textit{Professional}-Regel wurde sich an der \textit{Ubiquitous Language} aus \ac{DDD} orientiert. Ein Beispiel hierzu ist die Bezeichnung der Testklassen, indem domänenspezifische Titel wie \href{}{\code{DateValidationTest}} oder \href{}{\code{UnitOfMeasureValueValidationTest}} genutzt wurden.
...
Ein Beispiel hierfür ist in \href{}{\code{DateValidationTest}}, indem die Methoden wie beispielsweise \href{}{\code{checkValidatorForInvalideFebruaryDate()}} oder \href{}{\code{checkValidatorForFebruaryDateInLeapYear()}} den gezielten Test beschreiben.

\subsection*{Unit-Tests mit Einsatz von Mocks}
Zuzüglich zu den erläuterten Unit Tests testet die Testklasse \href{}{\code{UpdateConsumerGoodsTest}} das Ändern von Attributen einer Instanz der Klasse \href{}{\code{ConsumerGoods}} mit Hilfe eines Mocking-Werkzeugs.
Der Test bildet den Ablauf der Methode \href{}{\code{updateConsumerGoods()}} in der Klasse \href{}{\code{ConsumerGoodsManager}} ab.
Dabei wird eine im Test erzeugte Instanz der Klasse \href{}{\code{ConsumerGoods}} mit den Attributen eines neuen, im Test gemockten, Objekts der Klasse \href{}{\code{ConsumerGoods}} aktualisiert.
...
Zu einem älteren Entwicklungsstand wurde in der Testklasse \href{}{\code{StoreConsumerGoodsTest}} ebenfalls Mocks zum Testen angewandt, da die Überprüfung des \ac{EAN}-Codes zu dem Zeitpunkt in \href{}{\code{ConsumerGoodsBuilder}} stattfand und somit ein eine Instanz des \href{}{\code{ConsumerGoodsRepository}} übergeben werden musste.
...
Als Beispiel für die Erfüllung der \textit{Automic}-Regel dient ebenfalls der Einsatz von Assertions und das statische Programmieren von Ergebnissen, wie in \href{}{\code{UpdateConsumerGoodsTest}} dargestellt.
...
Die \textit{Thorough}-Regel wurde an diesem Beispiel eingehalten, indem die Tests die Grundfunktionalitäten der Anwendung, wie durch den Test \href{}{\code{UpdateConsumerGoodsTest}} das Aktualisieren eines Konsumguts, mit \href{}{\code{StoreConsumerGoodsTest}} das Einlagern eines Konsumguts und mit \href{}{\code{ConsumerGoodsGuiControllerTest}} die Interaktionsschnittstelle zur \ac{GUI} testen.
Ein weiteres Beispiel für das Einhalten der \textit{Repeatable}-Regel ist ebenfalls im \textit{Arrange}-Teil des Tests \href{}{\code{ConsumerGoodsGuiControllerTest}} dargestellt.
...
Ein Beispiel ist die Testklasse \href{}{\code{StoreConsumerGoodsTest}}, welche die Validierung der Attribute zum Anlegen eines Konsumguts testet.
Durch die Entkopplung zueinander können die Tests ebenfalls unabhängig und in beliebiger Reihenfolge ausgeführt werden.
Alle nötigen Informationen zum unabhängigen Durchführen des Tests sind im \textit{Assert}-Teil des Tests definiert, wie in \href{}{\code{ConsumerGoodsGuiControllerTest}} beispielhaft dargestellt.
...
Bei den Testmethoden wurde ebenfalls eine klare Bezeichnung gewählt, die das darauf folgende Testergebnis verständlich macht.
Ein Beispiel hierzu findet sich in \href{}{\code{StoreConsumerGoodsTest}}, indem die Methoden \href{}{\code{checkStoreValidConsumerGood()}} oder \href{}{\code{checkStoreConsumerGoodWithoutFoodDescription()}} die domänenspezifische Bezeichnung verwenden und somit für Domänenexperten verständlich wird, was der Test überprüft, ohne den Testablauf im Detail zu verstehen.

\subsection*{Code Coverage}
Die Code Coverage-Ergebnisse nach Durchführen der Unit-Tests sind in Tabelle \ref{tab:code-coverage-full} dargestellt.
Bei Betrachtung der Testabdeckung ist zu erkennen, dass der Schwerpunkt der Tests auf den Klassen in der \textit{Domain}- sowie in der \textit{Abstraction}-Schicht auf den Validierungsklassen sowie der Erzeugung eines Konsumguts durch den \href{}{\code{ConsumerGoodsBuilder}} befindet.
Das ist entspricht auch den Testbereichen der Testklassen \href{}{\code{DateValidatorTest}}, \href{}{\code{DayDateTest}}, \href{}{\code{MonthDateTest}}, \href{}{\code{YearDateTest}} sowie \href{}{\code{UnitOfMeasureValueTest}}, die den dazugehörigen Validator testen.
...
Hinzu kommt das Testen der Attributvalidierung des \href{}{\code{ConsumerGoodsBuilder}}.
...
Eine Ausnahme stellt die Testklasse \href{}{\code{ConsumerGoodsGuiControllerTest}}, diese überprüft die \ac{HTTP}-Schnittstelle und die entsprechenden Rückmeldungen des Servers an die GUI. 

...
Die Klasse hat die einzige Aufgabe, Objekte der Klasse \href{}{\code{ConsumerGoods}} zu Objekten der Klasse \href{}{\code{ConsumerGoodsResource}} zu mappen und somit für die Interaktion mit der \ac{GUI} zu verwenden.
Das gleiche gilt für die Klasse \href{}{\code{FridgeToFridgeResourceMapper}} und \href{}{\code{FoodShelfToFoodShelfResourceMapper}}, deren Aufgabe das Mappen eines \href{}{\code{Fridge}}- oder \href{}{\code{FoodShelf}}-Objekt zu einer entsprechenden Resource ist.
...
Das \textit{Single Responsibility Principle}-Prinzip wurde hingegen bei den Klassen \href{}{\code{ConsumerGoodsManager}} sowie \href{}{\code{StorageManager}} verletzt. Die Klassen haben neben dem Suchen eines Objekts auch die Aufgabe der Erstellung sowie das Löschen eines Objekts.
In diesem Zusammenhang lässt sich ebenfalls eine Verletzung des \textit{Single Responsibility Principle}-Prinzip bei der Klasse \href{}{\code{ConsumerGoodsGuiController}} erkennen.
Die Interaktion mit der \ac{GUI} umfasst sowohl das Erstellen, als auch das Ausgeben der verfügbaren Objekte der Klasse \href{}{\code{ConsumerGoods}}.
Gleiches gilt für das Löschen eines Objektes.
Hierzu findet die gesamte Interaktion, die verschiedene Aufgaben enthält, in der Klasse \href{}{\code{ConsumerGoodsGuiController}} statt.
Die Verletzung des \textit{Single Responsibility Principle}-Prinzip trifft auch auf die Klasse \href{}{\code{StorageGuiInterface}} zu.
Auch hier findet die gesamte Interaktion mit der Schnittstelle, worunter das Ausgeben, Erstellen und Löschen von Objekten des implementierten Interfaces \href{}{\code{Storage}} zählt, in der einen Klasse statt.
...
Das \textit{Open-Closed-Principle} wurde bei den Klassen \href{}{\code{ConsumerGoodsResource}}, \href{}{\code{FridgeResource}} und \href{}{\code{FoodShelfResource}} sowie bei den weiteren Resourcen-Klassen eingehalten.
...
Eine Verletzung des \textit{Open-Closed-Principle} stellen die Klassen \href{}{\code{ConsumerGoodsGuiController}} und \href{}{\code{StorageGuiController}} dar.
...
Es ist eine Vererbung der Superklasse \href{}{\code{UnitOfMeasure}} und den Subklassen \href{}{\code{Volume}}, \href{}{\code{Weight}} und \href{}{\code{Quantity}} implementiert.
Bei \href{}{\code{UnitOfMeasure}} handelt es sich um eine abstrakte Klasse.
...
Das Gleiche gilt für die Implementierung der Vererbung der Superklasse \href{}{\code{Storage}} und den Subklassen \href{}{\code{Fridge}} und \href{}{\code{FoodShelf}}.
Auch in diesem Fall ist die Klasse \href{}{\code{Storage}} abstrakt und die Subklassen überschreiben keine Funktionalität der Superklasse, wodurch das \textit{Liskov Substitution Principle} eingehalten wird.
...
Diese sind auch an dieser Stelle für die Superklassen \href{}{\code{UnitOfMeasure}} und \href{}{\code{Storage}} vorgesehen gewesen, allerdings führt das bei der Persistierung durch Hibernate zu Fehlern, da Hibernate die Referenz auf den direkten Typen benötigt und dieser bei Interfaces für Hibernate nicht ermittelbar ist.

\subsubsection*{Interface Segregation Principle}
Ein Positivbeispiel der Umsetzung des \textit{Interface Segregation Principle} zeigt sich in den Interfaces \href{}{\code{FridgeRepository}} und \href{}{\code{FoodShelfRepositoy}}.
...
Diesen Negativpunkt betrifft ebenfalls das Interface \href{}{\code{ConsumerGoodsRepository}}.

\subsubsection*{Dependency Inversion Principle}
Die Einhaltung des \textit{Dependency Inversion Principle} war bei den Klassen \href{}{\code{ConsumerGoods}} sowie \href{}{\code{Food}} und dem Interface \href{}{\code{StorableGood}} geplant.
Die Klasse \href{}{\code{ConsumerGoods}} referenziert dabei auf ein Objekt des Typs \href{}{\code{StorableGood}}. 
Das hätte zur Folge, dass unter Anwendung der Dependency Injection eine Erweiterung weiterer lagerbarer Objekte neben Essen möglich ist, ohne die Klasse \href{}{\code{ConsumerGoods}} hierzu anpassen zu müssen. 
Die Klasse \href{}{\code{ConsumerGoods}} referenziert dabei auf ein Objekt des Typs \href{}{\code{StorableGood}}.

Gleiches gilt für die Umsetzung der Superklassen \href{}{\code{Storage}} und \href{}{\code{UnitOfMeasure}} als Interfaces in Bezug auf die Klasse \href{}{\code{ConsumerGoods}}.
Auch hierbei könnten neue Lagermöglichkeiten oder Maßeinheiten hinzugefügt werden, ohne dass die Klasse \href{}{\code{ConsumerGoods}} dafür angepasst werden muss.
...
Daher ist zum aktuellen Zeitpunkt noch das referenzierte Gut vom Typ \href{}{\code{Food}} sowie \href{}{\code{Storage}} und \href{}{\code{UnitOfMeasure}} als Superklassen implementiert.

Darüber hinaus sind die Interfaces \href{}{\code{ConsumerGoodsRepository}}, \href{}{\code{FridgeRepository}} und \href{}{\code{FoodShelfRepository}} ein weiteres und implementiertes Beispiel des \textit{Dependency Inversion Principle}.
Die Interfaces definieren alle nötigen Methoden und die Services \href{}{\code{ConsumerGoodsManager}} und \href{}{\code{StorageManager}} haben zur Laufzeit eine Abhängigkeit auf ein Objekt, welches das entsprechende Interface implementiert hat.
...
Eine Verletzung des \textit{Dependency Inversion Principle} ist in den Mapper Klassen der Ressourcen-Mapper-Klassen zu finden. Die Verwendung der Klassen \href{}{\code{ConsumerGoodsToConsumerGoodsResourceMapper}}, \href{}{\code{FridgeToFridgeResourceMapper}} sowie \href{}{\code{FoodShelfToFoodShelfResourceMapper}} hängt direkt von den jeweiligen Instanzen der Mapper-Klassen ab.
...
Die Interaktion findet über die Klasse \href{}{\code{ConsumerGoodsGuiController}} und \href{}{\code{StorageGuiController}} statt.
...
Eine geringe Kopplung wurde bei der Implementierung ebenfalls erzielt, indem die Repository-Klassen \href{}{\code{ConsumerGoodsRepository}}, \href{}{\code{FridgeRepository}} und \href{}{\code{FoodShelfRepository}} bei der Rückgabe der instanziierten Objekte diese nicht als Liste vom Typ \textit{List} zurückgeben sondern als \textit{Iterable}.
...
Eine stärkere Kopplung ist in den Serviceklassen \href{}{\code{ConsumerGoodsManager}} und \href{}{\code{StorageManager}} zu finden.
Zum Löschen, Persistieren oder der Rückgabe initialisierter Objekte der Klassen \href{}{\code{ConsumerGoods}}, \href{}{\code{Fridge}} oder \href{}{\code{FoodShelf}} werden ausschließlich die in den entsprechenden Repository-Interfaces definierten Klassen verwendet.
Für den Zugriff auf eine Referenz des jeweiligen implementierten Repository-Types wird durch eine \href{}{\code{Dependency Injection}} an dieser Stelle ermöglicht.
...
Ein Beispiel mit einer starken Kopplung ist der statische Methodenaufruf \href{}{\code{validate()}} der Klasse \code{DateValidator}.
Auch in diesem Fall entsteht durch den statischen Methodenaufruf eine starke Kopplung an die Klasse \href{}{\code{DateValidator}}.
...
Ein Beispiel für eine hohe Kohäsion ist in der Klasse \href{}{\code{ConsumerGoods}} zu sehen.
Die Bestandteile eines Konsumguts sind das Lebensmittel und die dazugehörige Menge samt Maßeinheit.
Das Lebensmittel wird dazu in der Klasse \href{}{\code{Food}} verwaltet, während die Menge in einem Objekt des implementierten Interface-Typs \href{}{\code{UnitOfMeasure}} verwaltet ist.
Die Klasse \href{}{\code{Food}} hat als Attribut eine Instanz der Klasse \href{}{\code{BestBeforeDate}} zur Repräsentation des Mindesthaltbarkeitsdatums.
In der Klasse \href{}{\code{BestBeforeDate}} werden auch datumsspezifische Operationen wie zum Beispiel die Datumsüberprüfung mithilfe des \href{}{\code{DateValidators}} übernommen.
Das Objekt der Superklasse \href{}{\code{UnitOfMeasure}} repräsentiert die Menge in Form der spezialisierte Klassen \href{}{\code{Weight}}, \href{}{\code{Volume}} oder \href{}{\code{Quantity}}.
Der Wert der Menge wird in den Klassen durch das Attribut vom Typ \href{}{\code{Value}} verwaltet.

Das Ziel durch Schaffung einer hohen Kohäsion ist hierdurch, dass die jeweiligen Bestandteile des \href{}{\code{ConsumerGoods}} in einzelnen Klassen verwaltet werden, dadurch teilt sich der Code auf die entsprechenden Schwerpunkte auf und wird, gerade in Bezug auf Übergabe- und Rückgabeparameter, lesbarer.

Ein weiteres Beispiel für hohe Kohäsion ist die Klasse \href{}{\code{BestBeforeDate}}.
Das Mindesthaltbarkeitsdatum wird durch die Variablen des Typs \href{}{\code{DateOfYear}} und \href{}{\code{Year}} repräsentiert.
\href{}{\code{DateOfYear}} verwaltet das Datum eines Jahres durch die Attribute vom Typ \href{}{\code{Day}} und \href{}{\code{Month}}.
...
Darüber hinaus ist nicht die Klasse \href{}{\code{BestBeforeDate}} für eine genaue Spezifizierung oder Anpassung der einzelnen Attribute verantwortlich sondern die jeweiligen spezifischen Klassen.
...
Wie im Beispiel \href{}{\code{ConsumerGoods}} zu erkennen, reicht die Deklarierung \textit{@Table} aus.

Das gleiche betrifft die Bezeichnung der Spalten entsprechend der Attributbezeichnung.
Ebenfalls im Beispiel \href{}{\code{ConsumerGoods}} zu erkennen, reicht die Deklarierung \textit{@Column} aus.
...
Eine Nichteinhaltung des \ac{DRY}-Prinzip findet sich in der Resourcen-Klassen \href{}{\code{ConsumerGoodsReource}}, \href{}{\code{FridgeResource}}, \href{}{\code{FoodShelfResource}}, \href{}{\code{FoodResource}} und \href{}{\code{BestBeforeDateResource}} vor.

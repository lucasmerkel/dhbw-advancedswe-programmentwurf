...
\todo[]{Neues Naming der Klassen}
\todo[]{Annotations in Domain-Schicht}
\todo[]{Dependency Inversion Code Links}
\todo[]{StorableGoods als Interface einbauen}
...
Die Klasse \textit{StorageManager} hat die gleiche Applikationslogik wie die Klasse \textit{ConsumerGoodsManager}, jedoch mit dem Unterschied, dass es sich auf \textit{Fridge}-Objekte sowie \textit{FoodShelf}-Objekte bezieht.
...
Die Bridge dient dabei als Bindeglied zwischen dem internen Repository sowie der durch das Spring Framework bereitgestellten Oberklasse \textit{JpaRepository}, welche für die spätere Anbindung an eine Datenbank zur Persistierung benötigt wird.
...
Wäre diese Einschränkung für das Persistieren nicht gegeben, könnten \code{UnitOfMeasure} und \code{Storage} auch als Interface realisiert werden.